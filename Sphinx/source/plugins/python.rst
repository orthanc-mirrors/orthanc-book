...
<https://orthanc.uclouvain.be/hg/orthanc/file/Orthanc-1.12.11/OrthancServer/Plugins/Include/orthanc/OrthancCPlugin.h>`__
...
Note however, that, as of Orthanc 1.12.11, the Orthanc plugin SDK
...
* The call to ``orthanc.ExtendOrthancExplorer()`` installs the button
  ...
  <https://demos.jquerymobile.com/1.1.0/>`__ (as of Orthanc 1.12.11,
...
configuration.  Changing the ``Stable`` status will also trigger the lua callbacks
and python plugins that are listening on the ``STABLE_STUDY/STABLE_SERIES/..`` events.
