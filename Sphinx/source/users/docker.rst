...
Orthanc to be run::
  ...
  $ docker run -p 4242:4242 -p 8042:8042 --rm jodogne/orthanc:1.7.4
...
Or you can also start a specific version of Orthanc for more stability::
  ...
  $ docker run -p 4242:4242 -p 8042:8042 --rm jodogne/orthanc-plugins:1.7.4
...
Python 3.7 interpreter. Here is how to start this image::
  ...
  $ docker run -p 4242:4242 -p 8042:8042 --rm jodogne/orthanc-python:1.7.4
...
the `YAML file format <https://en.wikipedia.org/wiki/YAML>`__)::
  ...
  services:
    orthanc:
      image: jodogne/orthanc-plugins:1.7.4
...
container to some path in the filesystem of your Linux host, e.g.::
  ...
  $ docker run -p 4242:4242 -p 8042:8042 --rm -v /tmp/orthanc-db/:/var/lib/orthanc/db/ jodogne/orthanc:1.7.4 
...
* This command launches the mainline version. To start a released version,
  use e.g. ``jodogne/orthanc-debug:1.7.4``.
