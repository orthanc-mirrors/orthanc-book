...
This can be done with this sequence of commands::
  ...
  $ sudo service orthanc stop
  $ sudo wget https://lsb.orthanc-server.com/orthanc/1.8.2/Orthanc --output-document /usr/sbin/Orthanc
  $ sudo rm -f /usr/share/orthanc/plugins/*.so
  $ sudo wget https://lsb.orthanc-server.com/orthanc/1.8.2/libServeFolders.so --output-document /usr/share/orthanc/plugins/libServeFolders.so
  $ sudo wget https://lsb.orthanc-server.com/orthanc/1.8.2/libModalityWorklists.so --output-document /usr/share/orthanc/plugins/libModalityWorklists.so
  $
